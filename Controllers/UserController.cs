...
namespace WatersTicketingAPI.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<ActionResult<dynamic>> Authenticate([FromBody] UserLoginDto model, [FromServices] DataContext dbContext)
        {
            ...
            try
            {
                User user = await dbContext.Users.AsNoTracking().Where(x => x.Username == model.Username && x.Password == model.Password).FirstOrDefaultAsync();
                if (user == null)
        ...
        public async Task<ActionResult<UserDto>> GetByUsername([FromRoute] string username, [FromServices] DataContext dbContext)
        {
            try
            {
                UserDto user = mapper.Map<UserDto>(await dbContext.Users.AsNoTracking().FirstOrDefaultAsync(x => x.Username == username));
                return (user == null) ? NotFound(new { message = "user not found." }) : Ok(user);
            }
            catch (Exception ex)
            {
